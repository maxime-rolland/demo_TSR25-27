...
| `search_kb` | `GET /Knowledgebase/Article` — `query` optional; with none given, lists up to `limit` (default 50) articles for the agent to read directly rather than filtering server-side |
...
| `get_ticket_solution` | `GET /Assistance/Ticket/{id}/Timeline/Solution` — simplified to `{content, author_name, date}` for the latest entry, or `None` if the ticket has no formal solution yet |
...
**KB capitalization using the wrong content (added after initial delivery, user-caught bug).** The "event: update, status=Solved" branch (Skill section 5 below) originally drafted `create_kb_article` from whatever the agent could recall — the ticket's original text plus its own earlier followups — with no tool to read the ticket's actual formal Solution record. This broke exactly when a human resolved a ticket differently than the bot's own prior (private, escalation-time) diagnosis: ticket #16 ("Je n'ai plus de réseau") was escalated with a generic network-troubleshooting note, then a human technician added the real root cause as a formal Solution ("2024 workstations have a WiFi driver issue tied to Windows Update KB5048685, fixed by rerunning Windows Update") — but the bot had no way to see that solution record, so the KB article it created ("Diagnostiquer une perte de connexion réseau sur un poste de travail") just repeated its own earlier, generic, wrong-in-this-case diagnosis instead of the actual fix. `get_ticket_solution` fixes this: the skill now reads it first and is told explicitly to prefer it over any of its own or anyone else's earlier guesses.

**KB search missing real matches due to literal substring matching (added after initial delivery, user-caught bug).** `search_kb`'s original design required the agent to guess an exact RSQL substring filter (`name=like="*<keyword>*"`) — but GLPI's `like` filter is a literal substring match with no fuzziness, so any wording difference silently drops a real match. Confirmed empirically: ticket #18 ("Wifi LAPTOP", "Mon Laptop DT2405 n'a plus de wifi...") described the exact same underlying issue as the KB article created from ticket #16 ("Rétablir le Wi-Fi après la mise à jour Windows KB5048685") — same asset-naming-convention year (DT24xx = deployed 2024), same root cause — but `content=like="*wifi*"` returns 0 results against that article's text (which says "Wi-Fi", with a hyphen) while `content=like="*Wi-Fi*"` returns 1; the agent had no way to know the article used the hyphenated spelling, so it reported no match and escalated instead of reusing the existing article. Fixed by making `search_kb`'s `query` optional (default `""`, `limit` default 50) and changing the recommended usage entirely: call it with no arguments to list existing articles and have the agent read them directly, judging relevance semantically instead of via a guessed literal keyword. The RSQL `filter` parameter still works if explicitly passed, for a KB large enough that reading everything stops being practical.
...
- **`event: "new"`** — call `get_ticket_images` first (always, regardless of whether the ticket text mentions an attachment) and factor in anything visible (error messages, codes, visual context) before deciding, then search the KB for a matching answer.
  ...
  - No confident match → `escalate_ticket` alone, with the diagnosis as the reason — it assigns the ticket to a human *and* posts that diagnosis as the private note in one call, replacing the earlier behavior of leaving an unassigned private note nobody might notice. No public-facing reply is sent, and the ticket is never auto-resolved, in this case. (User-caught bug, fixed after initial delivery: the first version of this branch called `add_followup(is_private=true, ...)` *and then* `escalate_ticket(reason=<the same text>)` — since `escalate_ticket` already posts its own private followup internally, this posted the identical diagnosis twice on the ticket. The user spotted the duplicate note live and reported it; fixed by removing the standalone `add_followup` call, since `escalate_ticket`'s built-in note already covers it.)
- **`event: "update"` where the ticket's status just became "Solved"** — call `get_ticket_solution` first (added after initial delivery, user-caught bug — see the note above the tool table) and treat its `content` as the authoritative resolution, overriding any earlier diagnosis; fall back to `get_ticket_followups` only if there's no formal solution recorded. Then check `search_kb` for an existing article covering the same issue. This branch fires as a natural side effect of the auto-resolution above (calling `add_solution` changes the ticket, which queues a fresh GLPI `update` webhook for the *next* cron pass) — by design, not a new mechanism: it finds the already-existing matching article and does nothing, avoiding a duplicate.
  - None found → draft and `create_kb_article` from the ticket's content + the **actual** solution (not a generic guess).
